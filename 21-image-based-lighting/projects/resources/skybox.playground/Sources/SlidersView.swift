...
public class SlidersView: NSView {
  ...
  public init(frame: CGRect, metalView: MetalView) {
    ...
    let turbiditySlider = NSSlider(
      value: 0,
      minValue: 0,
      maxValue: 1,
      target: metalView,
      action: #selector(metalView.turbiditySliderChanged(sender:)))
    ...
    let sunElevationSlider = NSSlider(
      value: 0,
      minValue: 0,
      maxValue: 1,
      target: metalView,
      action: #selector(metalView.sunElevationSliderChanged(sender:)))
    ...
    let scatteringSlider = NSSlider(
      value: 0,
      minValue: 0,
      maxValue: 1,
      target: metalView,
      action: #selector(metalView.scatteringSliderChanged(sender:)))
